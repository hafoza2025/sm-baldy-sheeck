...
async function exportSalesExcel() {
    ...
    try {
        ...
        // 1. تحميل الطلبات - ✅ مع payment_method
        const { data: orders, error: ordersError } = await supabase
            .from('orders')
            .select('id, created_at, order_type, status, total, payment_method')
        ...
        // 4. تحميل الوصفات من جدول recipes
        ...
        // 5. تحميل أسعار المكونات من جدول ingredients
        let ingredientsCostMap = {};
        let itemCostMap = {};
        ...
        if (recipes && recipes.length > 0) {
            ...
            console.log('📦 عدد المكونات:', ingredients?.length || 0);
        ...
        orderItems.forEach(item => {
            ...
            salesData.push({
                ...
                'طريقة الدفع': paymentLabels[order.payment_method] || 'كاش',  // ✅ أضيف هنا
        ...
        // إضافة الملخص
        salesData.push({}, 
            { 'رقم الطلب': '', 'التاريخ': '', 'الوقت': '', 'الصنف': '━━━━━━━━━━━━━━━━━━━━', 'الكمية': '', 'السعر': '', 'الإجمالي': '', 'تكلفة الوحدة': '', 'تكلفة المكونات': '', 'طريقة الدفع': '', 'النوع': '' },
            { 'رقم الطلب': '', 'التاريخ': '', 'الوقت': '', 'الصنف': '📊 الملخص المالي', 'الكمية': '', 'السعر': '', 'الإجمالي': '', 'تكلفة الوحدة': '', 'تكلفة المكونات': '', 'طريقة الدفع': '', 'النوع': '' },
            { 'رقم الطلب': '', 'التاريخ': '', 'الوقت': '', 'الصنف': '💰 إجمالي المبيعات', 'الكمية': '', 'السعر': '', 'الإجمالي': totalRevenue.toFixed(2) + ' جنيه', 'تكلفة الوحدة': '', 'تكلفة المكونات': '', 'طريقة الدفع': '', 'النوع': '' },
            { 'رقم الطلب': '', 'التاريخ': '', 'الوقت': '', 'الصنف': '📦 تكلفة المكونات', 'الكمية': '', 'السعر': '', 'الإجمالي': totalIngredientsCost.toFixed(2) + ' جنيه', 'تكلفة الوحدة': '', 'تكلفة المكونات': '', 'طريقة الدفع': '', 'النوع': '' },
            { 'رقم الطلب': '', 'التاريخ': '', 'الوقت': '', 'الصنف': '✅ مجمل الربح', 'الكمية': '', 'السعر': '', 'الإجمالي': grossProfit.toFixed(2) + ' جنيه', 'تكلفة الوحدة': '', 'تكلفة المكونات': '', 'طريقة الدفع': '', 'النوع': '' },
            { 'رقم الطلب': '', 'التاريخ': '', 'الوقت': '', 'الصنف': '📈 هامش الربح', 'الكمية': '', 'السعر': '', 'الإجمالي': profitMargin + '%', 'تكلفة الوحدة': '', 'تكلفة المكونات': '', 'طريقة الدفع': '', 'النوع': '' }
        );
        ...
        ws['!cols'] = [
            ...
            { wch: 8 }, { wch: 10 }, { wch: 15 }, { wch: 12 }, { wch: 15 }, { wch: 12 }, { wch: 10 }
        ];
    ...
    try {
        ...
        if (expenses && expenses.length > 0) {
            ...
            expenses.forEach(exp => {
                totalGeneral += parseFloat(exp.amount) || 0;
                allExpenses.push({
                    'القسم': 'مصروف عام',
                    'النوع': expenseTypeNames[exp.expense_type] || exp.expense_type,
                    'التاريخ': new Date(exp.expense_date).toLocaleDateString('ar-EG'),
                    'المبلغ الكلي': exp.amount.toFixed(2),
                    'المدفوع': exp.amount.toFixed(2),
                    'المتبقي': '0.00',
        ...
        allExpenses.push({
            ...
            'النوع': '',
            'التاريخ': '',
            'المبلغ الكلي': totalGeneral.toFixed(2) + ' جنيه',
            'المدفوع': totalGeneral.toFixed(2) + ' جنيه',
            'المتبقي': '0.00 جنيه',
            'المدفوع إلى': '',
            'الوصف': ''
        });
        ...
        allExpenses.push({
            ...
            'النوع': '',
            'التاريخ': '',
            'المبلغ الكلي': (totalGeneral + totalInvoicesAmount).toFixed(2) + ' جنيه',
            'المدفوع': totalExpenses.toFixed(2) + ' جنيه',
            'المتبقي': totalInvoicesRemaining.toFixed(2) + ' جنيه',
            'المدفوع إلى': '',
            'الوصف': ''
        });
